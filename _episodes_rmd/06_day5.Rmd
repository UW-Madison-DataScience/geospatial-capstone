...
## Load vector data
The capstone data includes the Natual Earth Populated Places dataset. We have provided the dataset for you as a ".csv" file in the Capstone data folder. By the end of this section you should have a simple feature ('sf') data frame type of object in your Global Environment.
...
For the capstone data, we want to extract all the pixel values within a distance buffer of the two city points. Since our data is "unprojected" and therefor the units are in lat/long, the buffer distance will be in meters (hint: this information is in the Help file for the `extract` function!)
...
> You'll need to consider is the converstion from degrees to meters. The conversions depends on where you are on the globe. For example, at the equator, 1 degree is 111 km. This distance decreases as you move closer to the poles.
>
>
> {: .source}
{: .callout}
